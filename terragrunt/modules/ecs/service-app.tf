module "ccs_intranet_service" {
  source = "../ecs-service"

  add_security_group_roles = false
  ...
  container_port         = local.app_listening_port
  cpu                    = var.cpu
  desired_count          = var.desired_count - 1 # Consider the continuously running single leader, in charge of running migrations
  ecs_alb_arn            = var.ecs_alb_arn
  ecs_alb_sg_id          = var.ecs_alb_sg_id
  ecs_service_base_sg_id = var.ecs_service_base_sg_id
  family                 = local.family_app
