resource "aws_vpc_endpoint" "s3" {
  ...
  tags = merge(
    var.tags,
    {
      Name = title("${var.environment} S3 VPC Endpoint")
    }
  )
}
...
resource "aws_vpc_endpoint" "ecr_api" {
  ...
  tags = merge(
    var.tags,
    {
      Name = title("${var.environment} ECR API VPC Endpoint")
    }
  )
}
...
resource "aws_vpc_endpoint" "ecr_dkr" {
  ...
  tags = merge(
    var.tags,
    {
      Name = title("${var.environment} ECR Docker VPC Endpoint")
    }
  )
}
...
resource "aws_vpc_endpoint" "logs" {
  ...
  tags = merge(
    var.tags,
    {
      Name = title("${var.environment} Logs VPC Endpoint")
    }
  )
}
...
resource "aws_vpc_endpoint" "secrets" {
  ...
  tags = merge(
    var.tags,
    {
      Name = title("${var.environment} Secret Manager VPC Endpoint")
    }
  )
}
